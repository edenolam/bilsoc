...
(function ($) {
    ...
    function numeric_allowChar(validatedStringFragment, Char, settings) {
        ...
        if (Char == DEC_SEP || Char == DEC_SEP2) {
            // Only one decimal separator allowed
            if (validatedStringFragment.indexOf(DEC_SEP) >= 0)
                return false;
            if (validatedStringFragment.indexOf(DEC_SEP2) >= 0)
                return false;
    ...
    function isMaxDecimalsReached(string, settings) {
        ...
        var indexOfDecimalPoint2 = string.indexOf(DEC_SEP2);

        if (indexOfDecimalPoint == -1) {
            return false;
        } else if (indexOfDecimalPoint2 == -1) {
            return false;
        }
//        if (indexOfDecimalPoint == -1 || indexOfDecimalPoint2 == -1)
//return false;

        var decimalSubstring1 = string.substring(indexOfDecimalPoint);
        var decimalSubstring2 = string.substring(indexOfDecimalPoint2);

        var numDecimals1 = countDigits(decimalSubstring1);
        var numDecimals2 = countDigits(decimalSubstring2);

        if (numDecimals1 >= maxDecimalPlaces)
            return true;
        if (numDecimals2 >= maxDecimalPlaces)
            return true;
    ...
    $.fn.alphanum.setNumericSeparators = function (settings) {
        ...
        if (settings.decimalSeparator2.length != 1)
            return;

        THOU_SEP = settings.thousandsSeparator;
        DEC_SEP = settings.decimalSeparator;
        DEC_SEP2 = settings.decimalSeparator2;
    }
