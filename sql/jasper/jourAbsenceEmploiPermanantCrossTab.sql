SELECT (
	...
	WHERE ID_BILASOCICONS = 87 
		AND r_m_a.CD_MOTIABSE IN ('ABS001','ABS003','ABS004')
) NB_JOUR,(
	SELECT COALESCE(SUM(COALESCE(R_1111,0) + COALESCE(R_1112,0) + COALESCE(R_1113,0) + COALESCE(R_1114,0)),0) NB_FONC
	FROM ind_111
	WHERE ID_BILASOCICONS = 87
) NB_FONC,
'Fonctionnaire' LB_TYPE,
'Taux d''absentéisme «compressible»' LB_ABSE,
'ABSCOMP' CD_ABSE
UNION
SELECT (
	SELECT COALESCE(SUM(COALESCE(R_21213,0) + COALESCE(R_21214,0)),0) NB_JOUR
	FROM ind_212_1
	JOIN ref_motif_absence r_m_a ON r_m_a.ID_MOTIABSE = ind_212_1.ID_MOTIABSE
	WHERE ID_BILASOCICONS = 87 
		AND r_m_a.CD_MOTIABSE IN ('ABS001','ABS003','ABS004')
) NB_JOUR,(
...
'Fonctionnaire' LB_TYPE,
'Taux d''absentéisme médical' LB_ABSE,
'ABSMEDI' CD_ABSE
UNION
SELECT (
	SELECT COALESCE(SUM(COALESCE(R_21213,0) + COALESCE(R_21214,0)),0) NB_JOUR
