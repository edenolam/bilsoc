/*
*	Précédure stockée permettant de récupérer les collectivités pour la gestion des sirets (ccdg, par départements)
*/
DELIMITER $$
DROP PROCEDURE IF EXISTS get_collectivite_gestion_siret_cdg $$
CREATE PROCEDURE get_collectivite_gestion_siret_cdg(IN `id_util` INT)
	BEGIN
		...
		INSERT INTO not_proccessed  
			...
			INNER JOIN utilisateur u ON u.ID_UTIL = u3_.ID_UTIL
			INNER JOIN utilisateur_droits ud ON ud.ID_UTILISATEUR_CDG = u3_.ID_UTILISATEUR_CDG AND ud.ID_CDG_DEPARTEMENT = c2_.ID_CDG_DEPARTEMENT 
			WHERE i0_.BL_ERREUR = 0
			AND u.ID_UTIL = id_util
			AND CONV(1100000, 2, 10) & ud.FG_DROITS = 96 
		;
		...
		SELECT DISTINCT i0_.LB_COLL AS lbColl, d1_.LB_DEPA AS lbDepa, d1_.CD_DEPA AS cdDepa, i0_.LB_ADRE AS lbAdre, i0_.LB_VILL AS lbVill, hc.NM_ANCI_SIRE AS ancienSiret, i0_.NM_SIRE AS nmSire 
			...
			WHERE hc.ID_NATURE_MAJ = 1 
				/* Vérification sur l'utilisateur cdg */
				AND u.ID_UTIL = id_util
				AND CONV(1100000, 2, 10) & ud.FG_DROITS = 96 
