...
CREATE PROCEDURE apa2cons_handitorial(idBilaSociCons INT, idColl INT, idEnqu INT)
COMMENT ''
LANGUAGE SQL
NOT DETERMINISTIC
CONTAINS SQL
...
BEGIN
	...
	INTO vQA17, vQA3, vQA511, vQA512, vQA513, vRA9, vRA91, vQA521, vRA101,  vQA522, vQA523, vQA62, vQA72, vQA8, vQA82
	FROM information_colectivite_agent
	WHERE ID_COLL = idColl
	AND ID_ENQU = idEnqu;

	INSERT INTO bsc_handitorial_questions_generales (Q_A3, Q_A17, ID_BILASOCICONS)
	VALUES (vQA3, vQA17,idBilaSociCons);
    ...
    INSERT INTO bsc_handitorial_inaptitude_et_reclassement ( Q_A511, Q_A512, Q_A513,  R_A9, R_A91, Q_A521,	R_A101,  Q_A522, Q_A523, Q_A62, Q_A72, Q_A8, Q_A82, ID_BILASOCICONS)
	VALUES ( vQA511, vQA512, vQA513, vRA9, vRA91,  vQA521, vRA101, vQA522, vQA523, vQA62, vQA72, vQA8, vQA82, idBilaSociCons);

    SELECT   vQA511, vQA512, vQA513,   vRA9, vRA91,  vQA521, vRA101, vQA522, vQA523, vQA62, vQA72, vQA8, vQA82;
    IF 
	...
	vQA82 IS NOT NULL
	THEN
            UPDATE bilan_social_consolide SET BL_INCO_HANDITORIAL_INAPTITUDE_ET_RECLASSEMENT = '4', MOYENNE_HANDITORIAL_INAPTITUDE_ET_RECLASSEMENT = '100' WHERE ID_BILASOCICONS = idBilaSociCons AND ID_ENQU = idEnqu AND ID_COLL = idColl;
    END IF;
	  ...
	  AS (
	    ...
	    CREATE TEMPORARY TABLE temp_apa2cons_handitorial_anciennete (INDEX(Q1))
		  ...
		  FROM temp_apa2cons_handitorial_anciennete t;
		    ...
		    ### Remplissage de handitorial metiers
		  ...
		  AS (
            ...
            ### Création et remplissage de la table temporaire
		    DROP TEMPORARY TABLE IF EXISTS temp_apa2cons_handitorial_inapt_et_recla_cadre_emplois;
		    CREATE TEMPORARY TABLE temp_apa2cons_handitorial_inapt_et_recla_cadre_emplois
		      ENGINE = MEMORY
		      AS (
		        SELECT
		          bsa.ID_CADREMPL AS Q3,
		        ...
		        WHERE bsa.ID_COLL = idColl
		          AND bsa.ID_ENQU = idEnqu
		          AND bsa.ID_CADREMPL IS NOT NULL
				  AND bsah.bl_avis_inaptitude_en_cours = 1
				  # GROUP BY bsa.ID_CADREMPL
		      );

		    ### Remplissage de handitorial cadre emplois
		    INSERT INTO bsc_handitorial_inapt_et_recla_cadre_emplois (ID_BILASOCICONS, ID_CADREMPL, CADRE_EMPLOI_H, CADRE_EMPLOI_F)
		    SELECT idBilaSociCons, ce.ID_CADREMPL,
		      SUM(CASE WHEN t.Q1 = 1 THEN 1 ELSE 0 END) AS CADRE_EMPLOI_H,
		      SUM(CASE WHEN t.Q1 = 2 THEN 1 ELSE 0 END) AS CADRE_EMPLOI_F
		    FROM ref_cadre_emploi ce
		  	JOIN ref_filiere f on f.ID_FILI = ce.ID_FILI
		  	LEFT JOIN temp_apa2cons_handitorial_inapt_et_recla_cadre_emplois t ON t.Q3 = ce.ID_CADREMPL
		    WHERE ce.bl_vali = 0
			GROUP BY idBilaSociCons, ce.ID_CADREMPL
		    ORDER BY ce.ID_CADREMPL;
		    ...
		    WHERE ce.bl_vali = 0;
                    ...
                    IF vCADRE_EMPLOI_H > 0 OR vCADRE_EMPLOI_F > 0 THEN
                        UPDATE bilan_social_consolide SET BL_INCO_HANDITORIAL_INAPT_ET_RECLA_CADRE_EMPLOIS = '4', MOYENNE_HANDITORIAL_INAPT_ET_RECLA_CADRE_EMPLOIS = '100' WHERE ID_BILASOCICONS = idBilaSociCons AND ID_ENQU = idEnqu AND ID_COLL = idColl;
                    END IF;
                    

			### Création et remplissage de la table temporaire
		    DROP TEMPORARY TABLE IF EXISTS temp_apa2cons_handitorial_inapt_et_recla_metiers;
		    CREATE TEMPORARY TABLE temp_apa2cons_handitorial_inapt_et_recla_metiers
		      ENGINE = MEMORY
		      AS (
		        SELECT
		          bsag.ID_METIER AS Q3,
		    ...
		    ### Remplissage de handitorial metiers
		    INSERT INTO bsc_handitorial_inapt_et_recla_metiers (ID_BILASOCICONS, ID_METIER, METIER_H, METIER_F)
		    SELECT idBilaSociCons, m.ID_METIER,
		      SUM(CASE WHEN t.Q1 = 1 THEN 1 ELSE 0 END) AS METIER_H,
		      SUM(CASE WHEN t.Q1 = 2 THEN 1 ELSE 0 END) AS METIER_F
			  FROM ref_metier m
  		    LEFT JOIN temp_apa2cons_handitorial_inapt_et_recla_metiers t on t.Q3 = m.ID_METIER
  		    GROUP BY idBilaSociCons, m.ID_METIER
  		    ORDER BY m.ID_METIER;
                    ...
                    SELECT
                        ...
                        SUM(CASE WHEN t.Q1 = 2 THEN 1 ELSE 0 END) AS vMETIER_F
                    INTO vMETIER_H, vMETIER_F
                    FROM temp_apa2cons_handitorial_inapt_et_recla_metiers t;

                    IF vMETIER_H > 0 OR vMETIER_F > 0 THEN
                        UPDATE bilan_social_consolide SET BL_INCO_HANDITORIAL_INAPT_ET_RECLA_METIERS = '4', MOYENNE_HANDITORIAL_INAPT_ET_RECLA_METIERS = '100' WHERE ID_BILASOCICONS = idBilaSociCons AND ID_ENQU = idEnqu AND ID_COLL = idColl;
                    END IF;


			### Création et remplissage de la table temporaire
		  DROP TEMPORARY TABLE IF EXISTS temp_apa2cons_handitorial_inapt_et_recla_temps_complets;
		  CREATE TEMPORARY TABLE temp_apa2cons_handitorial_inapt_et_recla_temps_complets (INDEX(Q1))
			ENGINE = MEMORY
		  AS (
			  SELECT
				bsa.CD_SEXE AS Q1,
			...
			### Remplissage de handitorial temps complet
			INSERT INTO bsc_handitorial_inapt_et_recla_temps_complets (ID_BILASOCICONS, TEMPS_COMPLET_H, TEMPS_COMPLET_F, TEMPS_NON_COMPLET_H, TEMPS_NON_COMPLET_F)
			SELECT idBilaSociCons,
				SUM(CASE WHEN t.Q1 = 1 AND t.Q17_1 = 1 THEN 1 ELSE 0 END) AS TEMPS_COMPLET_H,
				SUM(CASE WHEN t.Q1 = 2 AND t.Q17_1 = 1 THEN 1 ELSE 0 END) AS TEMPS_COMPLET_F,
				SUM(CASE WHEN t.Q1 = 1 AND t.Q17_1 = 0 THEN 1 ELSE 0 END) AS TEMPS_NON_COMPLET_H,
				SUM(CASE WHEN t.Q1 = 2 AND t.Q17_1 = 0 THEN 1 ELSE 0 END) AS TEMPS_NON_COMPLET_F
			FROM temp_apa2cons_handitorial_inapt_et_recla_temps_complets t;
			...
			FROM temp_apa2cons_handitorial_inapt_et_recla_temps_complets t;
                        ...
                        IF vTEMPS_COMPLET_H > 0 OR vTEMPS_COMPLET_F > 0 OR vTEMPS_NON_COMPLET_H > 0 OR vTEMPS_NON_COMPLET_F > 0 THEN
                            UPDATE bilan_social_consolide SET BL_INCO_HANDITORIAL_INAPT_ET_RECLA_TEMPS_COMPLETS = '4', MOYENNE_HANDITORIAL_INAPT_ET_RECLA_TEMPS_COMPLETS = '100' WHERE ID_BILASOCICONS = idBilaSociCons AND ID_ENQU = idEnqu AND ID_COLL = idColl;
                        END IF;
			...
			AS (
			  ...
			  WHERE bsa.ID_COLL = idColl
				AND bsa.ID_ENQU = idEnqu
				AND (bsa.BL_BOETH = 1 OR bsah.bl_avis_inaptitude_en_cours = 1)
			);
